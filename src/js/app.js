...
function BodyController($scope, $http, $sce, sources, tags){
  ...
  $scope.tagFilter = job => checkProps(job, "sources") && checkProps(job, "content");
  ...
  $scope.companyFilter = company => {
    return company.jobs.reduce((check, job) => !check ? false : $scope.tagFilter(job), true);
  };

  function checkProps(job, prop){
    return $scope.sources.filter(e => e.off).map(e => e.name).reduce((check, name) => {
