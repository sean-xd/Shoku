...
app.value("sources", ls.sources ? JSON.parse(ls.sources) : [
...
app.value("tags", ls.tags ? JSON.parse(ls.tags) : [
...
function BodyController($scope, $http, $sce, sources, tags){
  $scope.companies = [];
  ...
  $scope.jobFilter = job => (checkProps(job, "sources") && checkProps(job, "tags", 1) && checkProps(job, "tags", 2));
  ...
  $scope.companyFilter = company => {
    return company.jobs.reduce((check, job) => !check ? false : $scope.jobFilter(job), true);
  };

  function checkProps(job, prop, x){
    return $scope[prop].filter(e => e.off).map(e => e.name).reduce((check, name) => {
      if(!check) return false;
      if(x === 1) prop = "content";
      if(x === 2) prop = "title"
      var jobProp = (name === "java") ? job[prop].replace(/javascript/g, "") : job[prop];
      return jobProp.toLowerCase().indexOf(name) === -1;
    }, true);
  }

  document.addEventListener("scroll", e => {
    var almostBottom = document.body.scrollTop > document.body.scrollHeight - document.body.clientHeight - 100,
...
app.controller("BodyController", ["$scope", "$http", "$sce", "sources", "tags", BodyController]);
